...
The shared styles in `client/src/index.css` use deep navy `#102A43`, warm ivory `#F7F4ED`, university green `#146B5A`, muted gold `#C9963B`, ink `#17212B`, and muted text `#5D6873`. Subtle warm borders and restrained shadows separate surfaces; gold is an accent, not the sole status cue. Georgia/Times provides the editorial display fallback and Segoe UI/Helvetica/Arial/system sans-serif supplies readable controls and body copy. No external font request is required.
