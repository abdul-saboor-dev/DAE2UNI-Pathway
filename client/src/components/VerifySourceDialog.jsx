...
export default function VerifySourceDialog({ record, onClose, onConfirm }) {
  ...
  return <dialog ref={dialogRef} onCancel={(event) => { if (pending) event.preventDefault(); else onClose() }} className="m-auto w-[min(94vw,34rem)] max-w-none border-t-4 border-gold bg-[var(--ui-paper)] p-5 shadow-2xl backdrop:bg-navy/70 sm:p-7">
    <form onSubmit={submit} className="space-y-4">
      ...
      {safeUrl ? <a href={safeUrl} target="_blank" rel="noopener noreferrer" className="block break-all text-sm text-academic underline">Open official source (external site)</a> : <p role="alert" className="break-all text-sm text-red-800">The current source URL is unsafe or missing. Edit the record first.</p>}
      ...
      <div className="flex flex-wrap justify-end gap-3"><button type="button" disabled={pending} onClick={onClose} className="min-h-11 rounded-xl border border-slate-300 px-4 font-bold focus-visible:ring-4 focus-visible:ring-academic">Cancel</button><button type="submit" disabled={pending || !confirmed || !safeUrl} className={adminButtonClass}>{pending ? 'Saving…' : 'Mark source verified'}</button></div>
    </form>
  </dialog>
}
