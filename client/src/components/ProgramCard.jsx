...
export default function ProgramCard({ program }) {
  ...
  return (
    <article className="paper-surface flex min-w-0 flex-col border-l-4 !border-l-gold p-5 sm:p-6">
      <div className="flex flex-wrap items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-academic">
      ...
      <p className="mt-2 font-bold text-academic">{program.university.name}</p>
      ...
      <Link to={`/programs/${encodeURIComponent(program.id)}`} className="mt-5 inline-flex min-h-11 items-center self-start font-black text-academic underline decoration-gold decoration-2 focus-visible:outline-3">
