...
export default function HomePage() {
  ...
  return <>
    <section className="border-b border-[var(--ui-border)] bg-navy text-white">
      <div className="site-container grid gap-10 py-12 sm:py-16 lg:grid-cols-[minmax(0,1.25fr)_minmax(18rem,.75fr)] lg:gap-16 lg:py-20">
        <div>
          ...
          <div className="mt-9 flex flex-wrap gap-3"><Link to={primary[0]} className="action-primary border border-white/40">{primary[1]}</Link><Link to="/universities" className="action-secondary !border-white/50 !text-white hover:!bg-white/10">Explore universities</Link></div>
        </div>
    ...
    <section className="site-container grid gap-10 py-12 lg:grid-cols-[16rem_1fr] lg:py-16" aria-labelledby="how-it-works">
      ...
      <ol className="border-t border-[var(--ui-border)]">{steps.map(([number, title, description]) => <li key={number} className="grid gap-3 border-b border-[var(--ui-border)] py-5 sm:grid-cols-[3rem_minmax(0,1fr)]"><span className="text-sm font-black text-academic">{number}</span><div><h3 className="text-lg font-black text-navy">{title}</h3><p className="body-copy mt-1 text-sm">{description}</p></div></li>)}</ol>
    </section>
