...
export default function AdminRecordList({ kind, definition, load, remove }) {
  ...
  return <>
    ...
    <div className="grid min-w-0 gap-6 xl:grid-cols-[16rem_minmax(0,1fr)]">
      <aside className="min-w-0"><details className="rounded-2xl border border-slate-200 bg-white p-4 xl:open" open><summary className="min-h-11 cursor-pointer font-bold text-slate-900">Filters</summary><div className="space-y-4 pt-4">
        ...
        <button type="button" onClick={list.clearFilters} className="min-h-11 w-full rounded-xl border border-slate-300 px-4 text-sm font-bold focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-400/50">Clear filters</button>
      </div></details></aside>
      <section id="admin-results" tabIndex="-1" className="min-w-0 focus:outline-none" aria-label={`${plural} results`}>
        ...
        {list.status === 'ready' && list.items.length > 0 && <div className="grid min-w-0 gap-3">
          {list.items.map((record) => <article key={record.id} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-lg font-black">{record.name || 'Untitled record'}</h2><p className="mt-1 break-all text-xs text-slate-600">{record.slug}{isUniversity ? ` · ${record.abbreviation || record.sector || ''}` : ` · ${record.university?.name || 'Unknown university'}`}</p></div><div className="flex flex-wrap gap-2"><StatusBadge value={record.recordStatus} /><StatusBadge value={record.source?.verificationStatus} /></div></div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3 text-sm font-bold"><Link className="min-h-11 rounded-lg border border-slate-300 px-4 py-3 text-teal-800 focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-400/50" to={`${base}/${record.id}/edit`}>Edit</Link>{isPublicRecord(record) && (isUniversity || isPublicRecord(parentRecords[record.university?.id])) && <Link className="min-h-11 rounded-lg px-4 py-3 text-teal-800 underline focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-400/50" to={isUniversity ? `/universities/${encodeURIComponent(record.slug || record.id)}` : `/programs/${encodeURIComponent(record.id)}`}>View public record</Link>}<button type="button" className="min-h-11 rounded-lg px-4 py-3 text-red-800 underline focus:outline-none focus-visible:ring-4 focus-visible:ring-red-400/50" onClick={() => { setDeleted(false); setTarget(record); setDeleteError('') }}>Delete</button></div>
          </article>)}
        </div>}
