...
const blankCampus = () => ({ clientKey: crypto.randomUUID(), name: '', city: '', district: '', address: '', isMainCampus: false, isActive: true })
const blankForm = () => ({ name: '', abbreviation: '', slug: '', sector: 'public', institutionType: 'general', establishedYear: '', recognitionBodies: '', campuses: [blankCampus()], contact: { websiteUrl: '', admissionsUrl: '', email: '', phone: '' }, source: { officialUrl: '', verificationStatus: 'unverified', lastVerifiedAt: '' }, recordStatus: 'draft' })
function fromRecord(record) {
  return {
    name: record.name || '', abbreviation: record.abbreviation || '', slug: record.slug || '', sector: record.sector || 'public', institutionType: record.institutionType || 'general', establishedYear: record.establishedYear ?? '', recognitionBodies: (record.recognitionBodies || []).join(', '),
...
export default function AdminUniversityFormPage() {
  ...
  if (state === 'not-found') return <div role="alert"><h1 className="text-2xl font-black">University not found</h1><Link to="/admin/universities" className="text-teal-800 underline">Back to universities</Link></div>
  ...
  return <>
    ...
    <form onSubmit={save} noValidate className="min-w-0 space-y-6">
      ...
      <fieldset className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6"><legend className="px-2 text-lg font-black">Campuses</legend><p className="mb-4 text-sm text-slate-600">Existing campus identities are preserved. Removing a campus used by a program will be blocked by the API.</p>{errors.campuses && <p role="alert" className="mb-4 text-sm font-bold text-red-800">{errors.campuses}</p>}
        ...
        <button type="button" disabled={form.campuses.length >= 30} className="mt-4 min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-bold focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-400/50 disabled:opacity-50" onClick={() => change('campuses', [...form.campuses, blankCampus()])}>Add campus</button>
      </fieldset>
      ...
      <div className="flex flex-wrap gap-3"><button type="submit" disabled={saving} className={adminButtonClass}>{saving ? 'Saving…' : 'Save university'}</button><Link to="/admin/universities" className="inline-flex min-h-11 items-center rounded-xl border border-slate-300 px-5 text-sm font-bold focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-400/50">Cancel</Link></div>
    </form>
  </>
}
