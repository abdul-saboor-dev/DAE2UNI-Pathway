...
export default function AdminDashboardPage() {
  ...
  return <>
    ...
    {state === 'ready' && <>
      ...
      <section aria-labelledby="dashboard-recent" className="mt-8"><div className="flex flex-wrap items-center justify-between gap-3"><h2 id="dashboard-recent" className="text-xl font-black">Recently updated</h2><div className="flex gap-3 text-sm font-bold"><Link className="text-academic underline" to="/admin/universities">Manage universities</Link><Link className="text-academic underline" to="/admin/programs">Manage programs</Link></div></div>{recent.length ? <ul className="mt-4 grid gap-3">{recent.map((record) => <li key={`${record.kind}-${record.id}`} className="flex min-w-0 flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4"><Link className="min-w-0 break-words font-bold text-academic underline" to={`/admin/${record.kind}/${record.id}/edit`}>{record.name}</Link><StatusBadge value={record.recordStatus} /></li>)}</ul> : <p className="mt-4 rounded-xl bg-white p-5 text-slate-600">No catalogue records have been created yet.</p>}</section>
    </>}
  </>
}
